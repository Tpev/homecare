...
class CaregiverCoverageMap extends Component
{
    ...
    public function render()
    {
        ...
        $maxCityCount = max(
            1,
            ...$cityRows->map(fn ($row): int => (int) $row->total)->all(),
        );

        $cityItems = $cityRows
            ->map(function ($row) use ($maxCityCount): array {
                $count = (int) $row->total;
                $intensity = $maxCityCount > 0 ? ($count / $maxCityCount) : 0.0;
