...
class ProfileRequestKnowledgeBaseImportService
{
    public function __construct(
        private readonly ProfileRequestKnowledgeBaseCatalog $catalog,
    ...
    public function plan(): array
    {
        ...
        return [
            'manifest_version' => ProfileRequestKnowledgeBaseCatalog::VERSION,
            'creates' => $creates,
            'noops' => $noops,
            'conflicts' => $conflicts,
            'counts' => [
    ...
    public function apply(User $actor): array
    {
        ...
        return DB::transaction(function () use ($actor): array {
            $plan = $this->plan();
            if ($plan['conflicts'] !== []) {
                throw new DomainException('Batch 5 KB import refused because governed content conflicts.');
            }
