...
class MarketplaceCareKnowledgeBaseImportService
{
    public function __construct(
        private readonly MarketplaceCareKnowledgeBaseCatalog $catalog,
    ...
    public function plan(): array
    {
        ...
        $existing = KnowledgeBaseEntry::query()->with('workingVersion.sources')
            ->whereIn('stable_id', MarketplaceCareKnowledgeBaseCatalog::APPROVED_STABLE_IDS)->get()->keyBy('stable_id');
        ...
        return [
            'manifest_version' => MarketplaceCareKnowledgeBaseCatalog::VERSION,
            'creates' => $creates,
            'noops' => $noops,
            'conflicts' => $conflicts,
    ...
    public function apply(User $actor): array
    {
        ...
        return DB::transaction(function () use ($actor): array {
            $plan = $this->plan();
            if ($plan['conflicts'] !== []) {
                throw new DomainException('Batches 6 and 7 KB import refused because governed content conflicts.');
            }
            ...
            foreach ($plan['creates'] as $stableId) {
                ...
                $entry = $this->workflow->createDraftWithStableId(
                    $actor, $stableId, $this->catalog->payload($definition), $this->catalog->sources($definition),
                );
    ...
    {
        $fields = ['type', 'title', 'answer_body', 'sensitivity', 'product_area', 'locale', 'roles',
    ...
    private function fingerprint(array $payload, array $sources): string
    {
        foreach ($payload as $field => $value) {
            if (is_array($value)) {
                $payload[$field] = array_values(array_unique(array_filter(array_map(static fn ($item): string => trim((string) $item), $value))));
