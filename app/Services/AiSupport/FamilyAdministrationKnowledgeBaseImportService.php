...
class FamilyAdministrationKnowledgeBaseImportService
{
    public function __construct(
        private readonly FamilyAdministrationKnowledgeBaseCatalog $catalog,
    ...
    {
        $creates = $noops = $conflicts = [];
        $existing = KnowledgeBaseEntry::query()->with('workingVersion.sources')
            ->whereIn('stable_id', FamilyAdministrationKnowledgeBaseCatalog::APPROVED_STABLE_IDS)->get()->keyBy('stable_id');
        ...
        return ['manifest_version' => FamilyAdministrationKnowledgeBaseCatalog::VERSION,
            'creates' => $creates, 'noops' => $noops, 'conflicts' => $conflicts,
    ...
    public function apply(User $actor): array
    {
        ...
        return DB::transaction(function () use ($actor): array {
            $plan = $this->plan();
            if ($plan['conflicts'] !== []) {
                throw new DomainException('Batches 8 and 9 KB import refused because governed content conflicts.');
            }
            ...
            foreach ($plan['creates'] as $stableId) {
                ...
                $entry = $this->workflow->createDraftWithStableId($actor, $stableId, $this->catalog->payload($definition), $this->catalog->sources($definition));
            ...
            return ['manifest_version' => FamilyAdministrationKnowledgeBaseCatalog::VERSION, 'created' => $created,
                'noops' => $plan['noops'], 'published_count_before' => $publishedBefore, 'published_count_after' => $publishedBefore];
        }, 3);
    }
    ...
    public function publishPackage(User $actor, string $reason): array
    {
        ...
        return DB::transaction(function () use ($actor, $reason): array {
            ...
            foreach ($this->catalog->entries() as $definition) {
                ...
                if ($version->status === KnowledgeBaseVersion::STATUS_PUBLISHED && (int) $entry->published_version_id === (int) $version->id) {
    ...
    {
        return hash_equals($this->fingerprint($this->catalog->payload($definition), $this->catalog->sources($definition)),
            $this->fingerprint($this->versionPayload($version), $version->sources->map(fn ($source): array => [
                ...
                'section_anchor' => $source->section_anchor, 'fact_supported' => $source->fact_supported,
            ])->all()));
    }
    ...
    {
        $fields = ['type', 'title', 'answer_body', 'sensitivity', 'product_area', 'locale', 'roles', 'membership_states',
            'route_target_ids', 'capability_ids', 'facts_may_state', 'facts_must_not_infer', 'next_actions',
            'escalation_conditions', 'retrieval_examples_match', 'retrieval_examples_no_match', 'evaluation_ids',
    ...
    private function fingerprint(array $payload, array $sources): string
    {
        foreach ($payload as $field => $value) {
            $payload[$field] = is_array($value)
                ? array_values(array_unique(array_filter(array_map(static fn ($item): string => trim((string) $item), $value))))
                : (is_string($value) ? trim($value) : $value);
        }
        $sources = array_values(array_map(static fn (array $source): array => [
            'source_id' => trim((string) ($source['source_id'] ?? '')), 'title' => trim((string) ($source['title'] ?? '')),
