...
The request contract follows the official [OpenAI function-calling guidance](https://developers.openai.com/api/docs/guides/function-calling) for strict schemas and the official [GPT-5.6 Luna model record](https://developers.openai.com/api/docs/models/gpt-5.6-luna). The initial interactive report conservatively estimated cost at $1.00 per million uncached input tokens, $0.10 per million cached input tokens, and $6.00 per million output tokens. `DEC-067` subsequently records and implements the current official August 14 rate of $0.20 / $0.02 / $1.20 as a distinct versioned catalog; the historical report remains unchanged and future runtime/rehearsal evidence uses the corrected catalog.
