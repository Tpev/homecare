...
| `FAM-STATE-001` | Read a normalized Family overview and targeted current states from authorized domain readers | Family | B/read | Batch 2 implemented; 40-row mass regression passing | Guided assistance Batch 1-2 | Safe payment-method, unresolved care-action, request/applicant, visit/change, submitted-hours, profile readiness/missing-step, unread-message, and history reads; no raw database/model access |
