...
| [`ADM-KB-001`](../11-admin-control-plane-and-pilot.md) | List, create, draft, edit, review, publish, pause, supersede, and safely delete KB entries | Authorized administrators | Platform control | Implemented; production lifecycle and 23 publications verified | Phase 1 | 23 reviewed non-pricing entries Published; held pricing entry Draft; customer AI release disabled |
...
| `FAM-STATE-001` | Read a normalized Family overview and targeted current states from authorized domain readers | Family | B/read | Batch 2 Family coverage implemented in source | Guided assistance Batch 1-2 | Safe payment-method, unresolved care-action, request/applicant, visit/change, submitted-hours, profile readiness/missing-step, unread-message, and history reads; no raw database/model access |
| `NAV-GUIDE-001` | Navigate to, focus, and accessibly highlight one registered UI control | Family first; caregiver later | B/guide | Batch 2 Family targets implemented in source | Guided assistance Batch 1-2 | Exact account-authorized request, plan, profile, conversation, billing, Care, Messages, and History targets; no arbitrary selectors, coordinates, or autonomous clicking |
...
| `CARE-REQUEST-004` | Explain an authorized care request's status and next action | Active family owner/member | A/B read-guide | Implemented in source | Guided assistance Batch 2 | Deterministic lifecycle, applicant attention, visit, hours, and care-payment target selection |
