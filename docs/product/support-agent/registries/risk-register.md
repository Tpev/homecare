...
| `RISK-COST-001` | Long conversations or loops create excessive cost/latency | Medium/high | Context limits, turn/retry ceilings, efficient baseline, alerts | Offline baseline complete: Luna p50/p95 2,182/3,498 ms and $0.06563460 for 278 calls; production shadow budget/alerts still required | Open |
| `RISK-MODEL-001` | Model/prompt change silently degrades behavior | High | Versioned candidate/prompt/schema/corpus/grader, repeated critical runs, compact report, full regression and shadow gate | Frozen-v4 Luna/Mini report: 556/556 calls, zero hard/critical failures, exact hashes; repeat on governed changes and complete shadow gate | Open |
