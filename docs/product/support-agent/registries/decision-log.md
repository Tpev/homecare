...
- Decision required: Select the lowest-cost current model/configuration that completes the identical 70-case synthetic corpus, has zero critical hard failures across five runs of every critical case, and passes the declared quality, grounding, navigation, and schema gates. The implemented comparison set is the deprecated `gpt-5-nano-2025-08-07` price-floor benchmark at low reasoning, `gpt-5.6-luna` at low reasoning, and `gpt-5.4-mini-2026-03-17` at low reasoning. Nano is measured but is not baseline-eligible. The complete schedule is 278 calls per candidate and 834 total. API credit and strict structured endpoint smoke checks now pass for all three models, but no configuration is approved until the complete repeated run and hard-failure review are recorded.
