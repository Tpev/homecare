...
- Implementation: Batch 1 is implemented in source as a deterministic payment-method intent/status fast path, active-Family-member safe reader, reusable guided-task lifecycle, semantic target coordinator, accessible guide strip/highlight, existing Stripe-flow continuation, authoritative completion verifier, and focused PHP/browser coverage. It does not change payment, fee, authorization, capture, or production-availability behavior.
- Batch 2 implementation: Implemented in source on August 17, 2026 as deterministic Family overview and targeted request/applicant, visit/change, submitted-hours/payment-attention, care-profile missing-step, message, regular-care-attention, and history reads. Resource-bound guide targets are re-authorized against the active Family Account, account facts bypass the model, and all domain mutations remain in the existing UI. The registry-driven mass evaluation passes all 40 Batch 1/2 rows, 120 representative phrasings, 10 collision cases, and 29 isolated application tests with 355 assertions and zero provider calls. Real-browser/accessibility expansion remains; production availability remains the same two-user pilot.
- Detailed contract: [App-aware guided assistance](../39-app-aware-guided-assistance.md)

### `DEC-075` - Approve Family Operations KB Wave 1 and correct payment-method permissions

- Status: Accepted
- Accepted: August 18, 2026 through Product instruction to author and implement the broader knowledge base
- Decision owner: Product
- Decision: Approve 50 new Family operations knowledge entries and one corrective `KB-FAM-004` revision. The package covers payments and recovery, requests and applicants, visits and submitted hours, care profiles and Family access, messages and notifications, regular care, and history. It maps 217 entry-to-intent links across 190 unique Family intent rows and carries five deterministic evaluations per definition.
- Payment correction: `DEC-041` is superseded only where it calls the saved-payment-method control owner-only. The current product permits any active Family member to use the secure shared Family payment-method flow. The Account owner remains the only person who can manage invitations, remove another member, transfer ownership, or close the Family Account. Payment credentials remain outside chat, and success still requires authoritative verification.
- Publication: Either full Administrator may import and publish the exact package alone through `ai-support:import-family-operations-kb`. Exact content conflicts fail closed, but no second-person review or separate per-entry ceremony is required. Publication changes neither availability nor pilot grants.
- Held scope: This decision does not publish pricing, map `FAM-PAY-028` or `FAM-PAY-029`, modify payment processing, add a write tool, or expand beyond the current two-user pilot.
- Detail: [Family Operations Knowledge Base Wave 1](../45-family-operations-kb-wave-1.md)

### `DEC-076` - Implement the Family operating layer and reversible preparation

- Status: Accepted
- Accepted: August 18, 2026 through Product instruction to implement Batch 3A and 3B in full
- Decision owner: Product
- Decision: Implement an executable disposition for all 324 Family intents; explicitly map all 190 Wave 1 knowledge-covered intents; add a state-aware Family chat start card; generalize active-task continuation, repetition recovery, action progress, and check-again behavior; introduce a generic authoritative completion-verifier interface; record content-free intent/task outcomes; expose searchable intent coverage and compact outcomes to Administrators; and implement five reversible preparation contracts for care profiles, request reuse, caregiver messages, submitted-hours correction/dispute, and support intake.
- Authority boundary: Catalog recognition does not grant action authority. Preparation does not save, send, approve, dispute, publish, or submit. User claims and page arrival are not completion proof. All records and resources are re-authorized; never-in-chat secrets are rejected; the model receives no generic ORM, SQL, database, arbitrary URL, selector, coordinate, browser, or write tool.
- Rollout: Source deployment and migration do not change Availability. The exact two-user Family pilot remains the only audience and **Everyone** remains off until an Administrator deliberately changes it.
- Unchanged product behavior: Pricing remains held, and no Stripe, payment authorization, capture, fee, payout, or ordinary domain workflow is changed. Batch 4 confirmed payment/time actions are excluded.
- Verification: The generated catalog validates 324 rows, 190 mappings, and 1,296 phrase definitions. The isolated mass runner passes 122 implemented phrases, 10 collision cases, and 42 application tests with 1,867 assertions and zero provider calls.
- Detail: [Family Operating Layer — Batch 3A and 3B](../46-family-operating-layer-batch-3.md)

## Product decision status

### `DEC-077` - Approve Batch 4 payment/time knowledge and pilot pricing truth

- Status: Accepted; deployed and published for the exact two-user pilot
- Accepted: August 18, 2026 through Product instruction to start Batch 4 while active shifts prevent deployment
- Decision owner: Product
- Decision: Approve the AI Support pricing truth that Families pay `$30/hour`, caregivers earn `$27/hour`, and LoLo receives `$3/hour`; author and publish a complete payment/submitted-hours KB package; explain a failed payment’s safe normalized reason when possible; add authorized payment/time reads, exact recovery destinations, deterministic pricing, and focused evaluations; and treat current application-pricing differences as a separate later code correction.
- Authority boundary: This decision changes support knowledge and explanation behavior only. It does not change Stripe, authorization, capture, refunds, payouts, booking prices, timesheet processing, payment records, visit records, or Availability. Raw provider errors and payment credentials remain outside chat. Secure authentication and consequential payment/time actions remain in the existing UI unless a separate confirmed tool is approved.
- Release outcome: Batch 3 and Batch 4 were deployed together at commit `0ab4eedb52c417040d53f2a5e03efb75a2d8f627`; all 18 exact Batch 4 entries were published. The exact two-user pilot remains the only audience and **Live for everyone** remains off.
- Package: 18 new stable entries, 90 linked evaluations, all 32 Family payment intents, 13 submitted-hours/correction intents, and 197 total executable-catalog mappings.
- Detail: [Payments, Submitted Hours, and Recovery — Batch 4](../47-payment-time-recovery-batch-4.md)

### `DEC-078` - Correct authenticated Batch 4 pilot findings without expanding availability

- Status: Accepted and implemented in source
- Accepted: August 18, 2026 through Product instruction to implement the authenticated production-audit corrections
- Decision owner: Product
- Decision: Make specific payment and submitted-hours language win over broad overview wording; recognize natural caregiver-submitted-hours questions; treat “question submitted hours” as reversible correction preparation; extract only the care receiver name from common profile-creation wording; invalidate discarded preparation cards and acknowledge discard; re-check the exact care payment before declaring a guided recovery complete; count actual human transfers accurately in Admin; and harden the optimistic-message UI against a null-state render race.
- Authority boundary: No new domain write, generic database access, card-data handling, payment retry, hours approval, Availability change, or pilot expansion is introduced. Payment completion is verified only when the exact authorized request or care plan no longer has an actionable payment state.
- Verification: 159 AI Support tests with 2,943 assertions, 28 focused tests with 1,687 assertions, a production frontend build, and seven isolated Chromium Family/chat/Admin scenarios pass.
- Detail: [Production Audit Corrections — Batch 4.1](../48-production-audit-corrections-batch-4-1.md)

### `DEC-079` - Approve Batch 5 care-profile and request-lifecycle ownership

- Status: Accepted and implemented in source; awaiting normal deployment, KB publication, and two-user pilot activation
- Accepted: August 18, 2026 through Product instruction to implement the complete next phase
- Decision owner: Product
- Decision: Publish 20 governed care-profile/request entries with 100 evaluations; explicitly map all 26 profile and 45 request intents; let the assistant read authorized profile/request state; collect and modify a profile over multiple turns; require a visible recap before consequential changes; create, update, make ready, make default, archive, and restore profiles through narrow confirmed tools; and support request status, applicant count, reuse, duplicate, replacement, copy, recovery, and confirmed withdrawal behavior.
- Lifecycle semantics: A live request change creates a reviewed replacement and preserves the original. A withdrawn or expired request is copied into a fresh private draft and never reopened in place. A request with an existing visit stays in the visit workflow. Permanent profile deletion and current-care impact review transfer to a person. Emergency, medical, and 24/7 boundaries are unchanged.
- Authority boundary: Actor, active Family membership, exact account/resource, writable fields, current revision/state, confirmation freshness, and idempotency are revalidated by the application. The model gets no generic database, ORM, arbitrary URL, selector, browser, or unrestricted write access. Completion requires a domain receipt plus a fresh authoritative read.
- Rollout: One simple activation command may enable Batch 5 only for the exact existing two-user pilot. It refuses any other active cohort and refuses to run while **Live for everyone** is on. Deployment and KB publication do not enable general release.
- Verification: The corrected Family mass harness passes 83 tests with 2,311 assertions; the complete AI Support suite passes 175 tests with 3,120 assertions; all 324 catalog records and 230 mappings validate; 137 established and 64 Batch 5 phrases pass with 10 protected collisions; the five-scenario interactive Chromium suite and all six responsive-chat scenarios passed at implementation, and the authenticated production correction adds focused no-console-error and archive-priority regressions.
- Production audit: Deployment, all 20 publications, exactly two capability extensions, and Pilot-only state were verified August 18, 2026. The new-user profile flow, multi-turn modifications, confirmation receipts, mobile chat, exact request-state read, navigation, and highlight worked. Three narrow corrections—composer resize, guidance result normalization, and explicit archive priority—were verified in source and require the normal follow-up deployment. No incorrect archive/readiness recap was confirmed.
- Detail: [Care Profiles and Request Lifecycle — Batch 5](../49-care-profiles-request-lifecycle-batch-5.md)

### `DEC-080` - Approve Batch 6/7 marketplace-care operation ownership

- Status: Accepted and implemented in source; production deployment, KB publication, pilot activation, and authenticated audit pending
- Accepted: August 20, 2026 through Product instruction to implement Batches 6 and 7 in full
- Decision owner: Product
- Decision: Author and publish 32 governed applicant/visit/regular-care entries with 160 linked evaluations; map all 25 Match, 35 Visit, and 26 Regular intents; add authorized readers and exact navigation; and permit narrow recap-confirmed tools for invitations, applicant decisions, conversations/messages, hiring, supported visit changes/cancellation/no-show/completion/hours/corrections/reviews/rebooking, and supported regular-care offers/counters/extra/skip/schedule/pause/resume/end operations.
- Financial truth: Support continues to state `$30/hour` paid by the Family, `$27/hour` earned by the caregiver, and `$3/hour` retained by LoLo. Batch 6/7 invokes existing booking and payment services but does not change Stripe pricing, fee, authorization, capture, payout, or refund policy code.
- Authority boundary: Every personalized read and write is re-authorized against the active Family Account. Writes require one exact resource, a material recap, a 30-minute confirmation, a fresh-state fingerprint, idempotency, an existing domain service, and an authoritative receipt. The model has no generic database, ORM, URL, selector, browser, or unrestricted tool access and cannot promise caregiver availability or message-read state.
- Human boundary: Profile/credential or blocking concerns; immediate/safety/serious visit problems; time-correction escalation; post-care disputes; completed-record alteration; completed-extra disputes; exceptional payment failure; and live-plan caregiver replacement transfer in the same support conversation without an automated finding.
- Rollout: `family_care_operations_v1` and all 27 tools default off. `ai-support:activate-batch7-pilot` refuses to run unless exactly the two configured pilot grants are active and Live for everyone is off. Deployment and KB publication do not change availability.
- Verification: 32 entries, 160 cases, 86 / 86 intents and 344 / 344 registered phrases, 237 total KB mappings, contract registration, cross-account isolation, recap/confirmation, invitation, applicant decision, exact messaging, hiring/payment setup, no-show, visit change, regular-care pause/resume, stale/idempotent behavior, receipts, exceptional transfer, and two-pilot-only activation are covered in source tests. The final source baseline is 204 / 204 AI Support tests with 4,231 assertions and an isolated 112 / 112 Family Batch 1–7 harness with 3,422 assertions. Production browser evidence remains pending.
- Detail: [Batch 6](../50-applicants-messaging-hiring-batch-6.md) and [Batch 7](../51-visits-regular-care-batch-7.md)

### `DEC-081` - Approve Batch 8/9 Family administration and human-owned Continuous Coverage

- Status: Accepted and implemented in source; production deployment, KB publication, pilot activation, and authenticated audit pending
- Accepted: August 20, 2026 through Product instruction to implement Batches 8 and 9 in full
- Decision owner: Product
- Decision: Author and publish 44 governed Account/Access/Communications/History/Coverage/Support entries with 220 linked evaluations; map every remaining Family catalog row so all 324 intents have an explicit KB mapping; add account-scoped readers; and permit ten narrow name, verification, Family invitation/membership, and notification tools under `family_administration_v1`.
- Continuous Coverage boundary: All 26 Continuous Coverage intents may be recognized and enriched with minimum authorized plan/shift context, but execution transfers to a human in the same conversation. No AI tool creates, assigns, changes, pauses, resumes, cancels, prices, charges, refunds, or closes a Continuous Coverage plan. Transfer never promises a queue position, response time, staffing availability, caregiver acceptance, or resolved care outcome.
- Security and privacy boundary: Credentials, verification/invitation tokens, payment data, provider errors, and cross-account records never enter chat. Signed-out recovery uses secure public/product routes. Account deletion, takeover, ownership transfer, privacy rights, formal record correction, disputes, safety/abuse/fraud findings, and unsupported exceptional outcomes remain human-owned.
- Confirmation: Except one low-risk notification read, writes require an exact resource, material recap, 30-minute confirmation, fresh-state assertions, idempotency, and an authoritative receipt. Owner-only membership authority is checked again at confirmation.
- Rollout: The capability and all ten tools default off. `ai-support:activate-batch9-pilot` refuses to run unless exactly the two configured pilot grants are effective and **Live for everyone** is off; it changes only those two grants. Deployment and publication do not enable general release.
- Verification: The source validates 72 Batch 8 and 46 Batch 9 intents, all 472 registered phrases, 44 entries, 220 evaluation cases, 324 / 324 catalog mappings, exact-account readers, confirmation/stale/idempotent behavior, receipts, protected-data denial, context-preserving transfer, absence of Continuous Coverage mutation tools, and exact-two-pilot activation. The complete AI Support suite passes 219 tests with 5,708 assertions and the isolated Family Batch 1–9 harness passes 127 tests with 4,899 assertions. Production browser evidence remains pending.
- Detail: [Batch 8](../52-family-administration-communication-records-batch-8.md) and [Batch 9](../53-continuous-coverage-exceptional-support-batch-9.md)

`DEC-072` is the current operating authority. Administrators choose Pilot only, Live for everyone, or Emergency stop from one Availability page. Historical readiness, evidence, preflight, and exact-commit decisions no longer gate operation. The two-user pilot remains the default after deployment until an Administrator selects Live for everyone. Role boundaries, emergency/24/7 handoff, explicit confirmation, human takeover, monitoring stops, and Human Only rollback remain enforced. `DEC-073` establishes the Family-intent coverage registry as the backlog and coverage source. `DEC-074` establishes app-aware read, guide, prefill, and verified-completion architecture; its payment-method Batch 1 and Family read/guide Batch 2 are implemented. `DEC-075` approves and publishes Family Operations Wave 1 without expanding availability. `DEC-076` implements the Batch 3 Family operating layer, explicit mappings, universal tasks/verifiers, Admin coverage, and five reversible preparation contracts. `DEC-077` approves and publishes the Batch 4 payment/time package and `$30 / $27 / $3` support truth. `DEC-078` corrects the authenticated pilot findings while preserving the exact same two-user boundary. `DEC-079` implements governed care-profile and request-lifecycle ownership with exact-pilot-only activation. `DEC-080` implements applicant, visit, and regular-care ownership. `DEC-081` implements Family administration and makes Continuous Coverage/exceptional outcomes a deterministic context-preserving human path. **Live for everyone** remains off.
