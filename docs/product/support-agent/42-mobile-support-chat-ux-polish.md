...
The support chat now behaves like a focused mobile conversation instead of a small desktop panel stretched onto a phone. It uses the full available visual viewport, opens at the latest message, keeps the composer stable above the mobile keyboard, and gives older users persistent access to human help and large touch targets.
...
| Latest message | Every mobile open and reopen scrolls to the newest message; desktop retains its prior reading-position behavior |
...
| Reading older history | Does not force-scroll; when new content arrives below, presents a 44px **New message** control |
...
| Composer feedback | Send icon changes to an accessible sending state while the request is in flight |
...
1. Opening the launcher on a phone must show the newest available message without a second swipe.
...
- `resources/js/support-chat.js`: mobile reopen position, Visual Viewport sizing, focus/selection preservation, Enter behavior, new-message detection, navigation sequencing
...
- `tests/e2e/specs/support-chat-responsive.spec.ts`: phone matrix, latest-message reopen, focus/cursor regression, Enter/Shift+Enter, long history, rotation, offline retry, and large text
...
| Support chat feature tests | 14 passed, 96 assertions |
| Responsive Chromium suite | 6 scenarios passed; includes 320, 360, 375, 390, and 430px phone widths |
