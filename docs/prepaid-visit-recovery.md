...
6. A separate `--release` preview requires a real completed visit after reset, consistent elapsed/worked/break time, explicit approval by the owning family, unchanged financial evidence, and an actual bill AND caregiver net amount that exactly match the retained payment. If hours change the bill, the hold stays in place for a separately approved adjustment. This command does not implement that adjustment.
