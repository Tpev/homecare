...
After registration, an unpaid booking for this pair that still lacks its agreement snapshot is blocked from authorization or capture until repaired. This also protects a visit missed by the repair or created concurrently with activation. Existing agreement snapshots continue to use their saved rates if the standard pricing version or rollout flag changes.
...
2. Run this read-only preview on the application host:
   ...
   Verify that the named family is Don and that the caregiver is the person who agreed to receive $15/hour. The command verifies Don's existing account email and prints the caregiver's user ID, source visit, future scheduled visits, corrected totals, and any records requiring separate review. It makes no Stripe calls.
...
5. Audit the historical booking IDs printed by the command, plus any other Don bookings since the billing update, for incorrect charges. The repair deliberately does not rewrite settled history. Verify a newly generated future visit has family rate 1575 cents, processing fee rate 0, caregiver rate 1500 cents, and policy `platform_pays_processing`.
...
Tests cover exact charges and transfers with nonzero processing fees, previews without writes, existing future visits, repeated repair, settled-payment exclusions, caregiver identity checks, pair isolation, stable snapshots, recurring generation, and completed extra visits.
