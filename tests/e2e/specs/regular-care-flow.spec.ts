...
test.describe('Recurring Care Experience', () => {
    test('family creation and management stay clear across desktop, tablet, and mobile', async ({ page }, testInfo) => {
        ...
        await page.getByText('Recurring care', { exact: true }).click();
        ...
        const planLink = page.locator('a[href*="/family/care/"]').filter({ hasText: 'Recurring care for E2E Recipient' }).first();
        await expect(planLink).toBeVisible();
        await planLink.click();
        await expect(page.getByRole('heading', { name: /Recurring care with E2E Ready Caregiver/i })).toBeVisible();
    ...
    test('caregiver sees direct offer and every real visit', async ({ page }, testInfo) => {
        ...
        await expect(page.getByText('Recurring care').first()).toBeVisible();
