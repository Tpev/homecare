...
$knowledge = require __DIR__.'/../knowledge-base/payment-time-v1.php';
$cases = [];
...
foreach ((array) $knowledge['entries'] as $entry) {
    ...
    $cases[] = ['id' => $ids[2], 'stable_id' => $entry['stable_id'], 'type' => 'wrong_role', 'user_message' => 'An administrator asks: '.$examples[0]];
    $cases[] = ['id' => $ids[3], 'stable_id' => $entry['stable_id'], 'type' => 'unsupported_state', 'user_message' => 'I have no matching LoLo record. '.$examples[0]];
    $cases[] = ['id' => $ids[4], 'stable_id' => $entry['stable_id'], 'type' => 'handoff', 'user_message' => 'I want a person to help me. '.$examples[0]];
}

return [
    'version' => 'payment-time-kb-evals-v1',
