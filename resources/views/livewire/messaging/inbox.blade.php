        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[72vh]">
            ...
            <section class="lg:col-span-8 xl:col-span-9 flex flex-col">
                @if ($active)
                    ...
                    <div
                        class="flex-1 overflow-y-auto px-4 py-5 bg-gradient-to-b from-white to-slate-50"
                        x-data="{ jump(){ this.$el.scrollTop = this.$el.scrollHeight } }"
                        x-init="$nextTick(() => jump())"
                        x-on:message-sent.window="jump()"
                        x-on:thread-refreshed.window="jump()"
                    >
                        <div class="space-y-3">
                    ...
                    <div class="border-t border-slate-200 p-4 bg-white">
                        ...
                        <form wire:submit="sendMessage" class="space-y-3">
                            ...
                            <div class="flex items-center justify-between">
                                <p class="text-xs text-slate-500">Real-time sync every 2 seconds.</p>
                                <x-button type="submit" color="blue" :disabled="!$canSend">Send message</x-button>
                            </div>
                        </form>
                    </div>
                @else
