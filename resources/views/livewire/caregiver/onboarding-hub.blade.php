<div class="hc-page py-6 sm:py-8 space-y-5">
        ...
        <div class="relative space-y-4">
            ...
            <div class="rounded-xl border border-white/20 bg-white/10 p-3">
                <div class="flex items-center justify-between gap-3">
                    <p class="text-xs uppercase tracking-[0.12em] text-slate-300">Required progress</p>
                ...
                <div class="mt-2 h-2 rounded-full bg-white/20">
                    <div class="h-2 rounded-full bg-cyan-300 transition-all duration-300" style="width: {{ $state['progress_percent'] }}%"></div>
                </div>
            </div>
        </div>
    </section>
    ...
    @if ($state['is_under_review'])
        <section class="rounded-2xl border border-sky-200 bg-sky-50 p-4">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-sm font-semibold text-sky-900">Profile under review</p>
                    <p class="mt-1 text-sm text-sky-800">Your profile is submitted. We review most profiles within 1 business day.</p>
                </div>
    ...
    <section class="space-y-3">
        <h2 class="font-display text-lg font-semibold text-slate-900">Required steps</h2>
        <div class="grid grid-cols-1 gap-3">
            @foreach ($state['required_steps'] as $step)
                <a href="{{ $step['route'] }}" wire:navigate class="block rounded-2xl border {{ $step['done'] ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-white' }} p-4 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <p class="text-xs uppercase tracking-[0.12em] {{ $step['done'] ? 'text-emerald-700' : 'text-slate-500' }}">
                                {{ $step['done'] ? 'Done' : 'Required' }} · ~{{ $step['minutes'] }} min
                            </p>
                            <p class="mt-1 text-base font-semibold text-slate-900">{{ $step['title'] }}</p>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4">
        <details>
            <summary class="cursor-pointer text-sm font-semibold text-slate-900">Optional profile boosters</summary>
            <div class="mt-3 grid grid-cols-1 gap-3">
                @foreach ($state['optional_steps'] as $step)
                    <a href="{{ $step['route'] }}" wire:navigate class="block rounded-xl border {{ $step['done'] ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50' }} p-3">
                        <div class="flex items-start justify-between gap-3">
                            <div>
