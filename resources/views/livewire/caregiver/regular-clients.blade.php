                ...
                <div class="space-y-3">
                    @forelse ($offers as $offer)
                        @php
                            ...
                            $offerPricing = $pricing->quoteForPair($offer->family_account_id, $offer->caregiver_user_id, 60);
                            $offerRate = $offerPricing['caregiver_gross_rate_cents'] / 100;
                        @endphp
                            ...
                            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-[#607080]">
                                        {{ $offer->family?->name }} - {{ $scheduleService->scheduleLabel($offer, $offer->status === \App\Models\CarePlan::STATUS_COUNTERED) }} - ${{ number_format($offerRate, 2) }}/hr{{ $offerPricing['caregiver_fee_policy'] === \App\Models\CarePricingAgreement::PLATFORM_PAYS_PROCESSING ? '' : '*' }}
                                    </p>
                                    @if ($offerPricing['caregiver_fee_policy'] === \App\Models\CarePricingAgreement::PLATFORM_PAYS_PROCESSING)
                                        <p class="mt-1 text-sm text-[#607080]">Agreed earnings. LoLo covers payment processing costs.</p>
                                    @endif
