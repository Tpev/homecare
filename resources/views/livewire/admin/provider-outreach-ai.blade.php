<div class="hc-page py-8 space-y-6">
    ...
    <x-card>
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div class="grid grid-cols-2 gap-2 text-center text-xs sm:grid-cols-5">
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                        <p class="font-semibold text-slate-900">{{ number_format($summary['draft']) }}</p>
                        <p class="text-slate-600">Waiting</p>
                    </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(22rem,0.8fr)]">
            ...
            <div class="space-y-4">
                <form wire:submit.prevent="uploadCsvBatch" class="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                    ...
                    <div class="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                        <x-input label="Batch label" placeholder="Wake County social workers" wire:model="batchLabel" />
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-700">CSV file</label>
                            <input type="file" wire:model="csvFile" accept=".csv,text/csv,text/plain" class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-slate-700 hover:file:bg-slate-200">
                            @error('csvFile')
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Active batch</p>
                    ...
                    @if($batchId)
                        <div class="mt-3 grid grid-cols-4 gap-2 text-center text-xs">
                            <div class="rounded-xl border border-emerald-200 bg-emerald-50 px-2 py-2 text-emerald-800">Good {{ $batchSummary['good'] }}</div>
                            <div class="rounded-xl border border-rose-200 bg-rose-50 px-2 py-2 text-rose-800">Bad {{ $batchSummary['bad'] }}</div>
                            <div class="rounded-xl border border-red-200 bg-red-50 px-2 py-2 text-red-800">Error {{ $batchSummary['error'] }}</div>
                            <div class="rounded-xl border border-amber-200 bg-amber-50 px-2 py-2 text-amber-800">IVR/VM {{ $batchSummary['ivr_vm'] }}</div>
                        </div>
                        <div class="mt-3 max-h-64 space-y-2 overflow-auto pr-1">
                            @foreach($batchCalls as $call)
                                @php
                                    $rating = data_get($call->metadata, 'provider_outreach_interaction_rating');
                                    $ratingTone = match ($rating) {
                                        ...
                                        'ivr_vm' => 'text-amber-700 bg-amber-50 border-amber-200',
                                        default => 'text-slate-600 bg-slate-50 border-slate-200',
                                    };
                                @endphp
                                <div class="rounded-xl border border-slate-200 px-3 py-2">
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="truncate text-sm font-semibold text-slate-900">{{ data_get($call->metadata, 'target_organization') ?: $call->to_phone }}</p>
                                        <span class="shrink-0 rounded-full border px-2 py-0.5 text-[11px] font-semibold {{ $ratingTone }}">
                                            {{ $rating ? str($rating)->replace('_', '/')->title() : strtoupper((string) $call->status) }}
                                        </span>
                                    </div>
                                    <p class="mt-0.5 truncate text-xs text-slate-500">{{ $call->to_phone }}{{ data_get($call->metadata, 'target_name') ? ' · '.data_get($call->metadata, 'target_name') : '' }}</p>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                        <div>
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                @endif
                                @if($rating)
                                    <x-badge :text="str($rating)->replace('_', '/')->title()" color="amber" />
                                @endif
                            </div>
