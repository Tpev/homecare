...
<section id="quick-request" class="hub-quick-request rounded-[1.75rem] border p-4 sm:rounded-[2rem] sm:p-6">
    <div class="flex items-start justify-between gap-3">
        <div>
            <p class="text-[11px] font-black uppercase tracking-[0.18em] text-blue-600">Quick request</p>
            <h2 class="mt-1 text-xl tracking-tight text-slate-950 sm:text-2xl">Start your care request now.</h2>
    ...
    <div class="mt-4 hidden grid-cols-3 gap-2 sm:grid">
        @foreach ($stepLabels as $index => $label)
            <div class="rounded-2xl border px-3 py-3 text-center {{ $step >= $index ? 'hub-step-active' : 'hub-step-idle' }}">
            ...
            <div class="grid gap-4">
                <div>
                    ...
                    <input id="recipient_name" type="text" wire:model.live="recipient_name" placeholder="Example: My mom, Margaret Johnson"
                        class="mt-2 block min-h-12 w-full rounded-2xl border border-slate-200 px-4 text-base shadow-sm focus:border-[#4F6FAF] focus:outline-none focus:ring-4 focus:ring-[#4F6FAF]/10">
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach ($taskOptions as $task)
                            ...
                            <button
                                ...
                                class="min-h-11 rounded-full border px-4 py-2 text-sm font-semibold transition {{ $active ? 'hub-pill-active' : 'hub-pill-idle hover:border-[#7C5DDC]/40 hover:bg-[#7C5DDC]/10' }}"
                            >
                                {{ $task['name'] }}
                            </button>
                        @endforeach
                ...
                <div>
                    ...
                    <textarea id="additional_info" rows="3" wire:model.live="additional_info" placeholder="Example: She mostly needs companionship, lunch prep, and someone around while I'm at work."
                        class="mt-2 block w-full rounded-2xl border border-slate-200 px-4 py-3 text-base shadow-sm focus:border-[#4F6FAF] focus:outline-none focus:ring-4 focus:ring-[#4F6FAF]/10"></textarea>
    ...
    @else
        <div class="mt-6 space-y-5">
            <div class="rounded-[1.5rem] border border-blue-200 bg-blue-50 p-5">
                ...
                <h3 class="mt-2 text-2xl tracking-tight text-slate-950">Create your account to publish it.</h3>
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            @if ($step < 3)
                <button
                    type="button"
                    wire:click="nextStep"
                    class="inline-flex min-h-12 items-center justify-center rounded-2xl bg-slate-950 px-6 text-sm font-bold text-white shadow-lg shadow-slate-900/20 transition hover:bg-[#4F6FAF]"
                >
                    Continue
                </button>
            @else
                <div class="grid w-full gap-3 sm:w-auto sm:min-w-[280px]">
                    <button
                        ...
                        class="inline-flex min-h-12 items-center justify-center rounded-2xl bg-blue-600 px-6 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:bg-[#4F6FAF]"
                    >
