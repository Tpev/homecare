<div class="space-y-6">
    <section class="hc-surface p-5 sm:p-6">
        ...
        <div class="mt-4 flex flex-col gap-5 sm:flex-row sm:items-start">
            ...
            <div class="min-w-0 flex-1"><h2 class="hc-care-title">{{ $plan->caregiver?->name ?: 'Caregiver not selected' }}</h2><p class="mt-1 hc-care-subtitle">{{ collect([$plan->caregiver?->city, $plan->caregiver?->state])->filter()->join(', ') }}</p><div class="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm">@if($caregiverProfile?->years_experience !== null)<span>{{ $caregiverProfile->years_experience }} years of experience</span>@endif @if($caregiverProfile?->reviews_count)<span>{{ number_format((float)$caregiverProfile->average_rating, 1) }} / 5 · {{ $caregiverProfile->reviews_count }} {{ Str::plural('review', $caregiverProfile->reviews_count) }}</span>@endif<span>${{ number_format($planCareRate, 2) }}/hour {{ $isPending ? 'proposed rate' : 'agreed rate' }}</span></div></div>
        </div>
