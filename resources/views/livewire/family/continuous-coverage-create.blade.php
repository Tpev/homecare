    ...
    <form wire:submit="save" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div class="space-y-6">
            ...
            <section class="rounded-3xl border border-[#DED6CA] bg-white p-5 sm:p-6">
                ...
                <div class="mt-5 grid gap-4 sm:grid-cols-2">
                    <div class="rounded-2xl border border-[#DED6CA] bg-[#F7F2EA] p-4"><p class="text-sm font-semibold text-[#324457]">Hourly price</p><p class="mt-1 text-xl font-semibold text-[#17313F]">${{ number_format($hourlyRate, 2) }}/hour*</p><p class="mt-1 text-xs text-[#607080]">Care ${{ number_format($hourlyRate, 2) }} + processing fee ${{ number_format(config('marketplace.pricing_v2.family_processing_fee_hourly_cents', 100) / 100, 2) }} = ${{ number_format($hourlyRate + config('marketplace.pricing_v2.family_processing_fee_hourly_cents', 100) / 100, 2) }}/hour total.</p><p class="mt-1 text-xs text-[#607080]">*The processing fee is charged only for completed time.</p></div>
            ...
            @if($step === 4)
                <section class="rounded-3xl border border-[#DED6CA] bg-white p-5 sm:p-6">
                    ...
                    <dl class="mt-5 grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="rounded-2xl bg-[#F7F2EA] p-4"><dt class="text-xs font-semibold text-[#7B8794]">Hourly price</dt><dd class="mt-1 font-semibold">${{ number_format($hourlyRate, 2) }}/hour care*</dd><dd class="text-sm text-[#526474]">+ ${{ number_format(config('marketplace.pricing_v2.family_processing_fee_hourly_cents', 100) / 100, 2) }}/hour processing fee.</dd></div>
