                    ...
                    @if ($savedReady)
                        @if ($affectedCare !== [])
                            <section class="rounded-2xl border border-amber-200 bg-amber-50 p-5" aria-labelledby="update-care-heading">
                                ...
                                <ul class="mt-3 space-y-2 text-sm text-amber-950">@foreach($affectedCare as $item)<li class="rounded-lg bg-white/70 px-3 py-2"><span class="font-semibold">{{ $item['title'] }}</span> &middot; {{ $item['type'] === 'coverage' ? 'Continuous Coverage' : ($item['type'] === 'regular' ? 'Recurring care' : 'Care request') }}</li>@endforeach</ul>
