...
func TestTwilioVoiceCanUseCallbackDiscoveryProfileViaQuery(t *testing.T) {
	server := testServer(config.Config{
		Port:                   "8088",
	...
	req := httptest.NewRequest(http.MethodPost, "http://localhost:8088/twilio/voice?prompt_profile=callback_discovery&voice_ai_call_id=42", strings.NewReader(form.Encode()))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	req.Header.Set("X-Twilio-Signature", twilioSignature("twilio-secret", "https://carelolo.com/twilio/voice?prompt_profile=callback_discovery&voice_ai_call_id=42", form))
